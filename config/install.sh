...
#
# To increase download speed apt source and Raspbian source are changed to
...
sudo apt update
echo -e '\e[32mapt source changed\e[0m'

# then install update-alternatives
sudo update-alternatives --install /usr/bin/pip pip /usr/bin/pip3 1
sudo update-alternatives --install /usr/bin/python python /usr/bin/python3 1
echo -e '\e[32mpython, pip changed to python3 and pip3\e[0m'

# change to tsinghua pip mirror and upgrade pip
pip config set global.index-url https://pypi.tuna.tsinghua.edu.cn/simple
pip install pip -U
echo -e '\e[32mpip source changed\e[0m'
...
echo -e '\e[32mopencv-contrib-python installed\e[0m'

# upgrade wiringPi to 2.52
wget https://project-downloads.drogon.net/wiringpi-latest.deb
sudo dpkg -i wiringpi-latest.deb
rm wiringpi-latest.deb
echo -e '\e[32mwiringPi upgraded'

# install and change shell to zsh
sudo apt install zsh -y
chsh -s /bin/zsh
echo -e '\e[32mchanged shell to zsh\e[0m'